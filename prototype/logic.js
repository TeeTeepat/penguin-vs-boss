/* Pure game rules from CONTEXT.md. No DOM. Loaded by index.html, tested by test.mjs. */
...
const heartsForLevel = (level) => 4 + Math.min(Math.max(level, 1), MAX_LEVEL);

const factKey = (f) => `${f.a}x${f.b}`;

/* Full pool is 2x2..12x12; Teacher narrows the table (first factor) range. */
function factPool(min, max) {
  const pool = [];
...
function pickFact(pool, weak, rng, avoidKey) {
...
function updateWeak(weak, key, correct) {
  const next = { ...weak };
  if (!correct) {
    next[key] = { streak: 0 };
    return next;
...
function applyAnswer(hearts, maxHearts, correct, resetOnWrong) {
  if (correct) return Math.max(0, hearts - 1);
  return resetOnWrong ? maxHearts : hearts;
}

const api = { MODES, MAX_LEVEL, heartsForLevel, factKey, factPool, pickFact, updateWeak, applyAnswer };
