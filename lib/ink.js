...
export function segmentStrokes(strokes) {
  const items = (strokes || [])
    .map((stroke, order) => {
      ...
      return { stroke, order, minX: b.minX, maxX: b.maxX };
    })
  ...
  for (const it of items) {
    ...
    if (last) {
      const gap = it.minX - last.maxX;
      const lastWidth = last.maxX - last.minX;
      const itemWidth = it.maxX - it.minX;
      const avgWidth = (lastWidth + itemWidth) / 2;
      if (gap <= 0 || gap < GAP_RATIO * avgWidth) {
        last.items.push(it);
    ...
    groups.push({ minX: it.minX, maxX: it.maxX, items: [it] });
  }
  return groups.map((g) =>
