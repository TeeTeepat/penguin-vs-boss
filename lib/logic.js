...
export const TOPICS = {
  ...
  mul3:     { timer: 120 },
  time:     { timer: 30 },
  /* Mix draws each question from the Student's playable topics; not in
     TOPIC_ORDER so it never gates or grants progression. Timer per question. */
  mix:      { timer: 30 },
};
...
export function genQuestion(topicKey, st, rng, avoidKey) {
  if (topicKey === "mix") {
    const pool = playableTopics(st);
    const pick = pool.length ? pool[Math.floor(rng() * pool.length)] : "add2";
    return genQuestion(pick, st, rng, avoidKey);
  }
  const q = (() => {
    switch (topicKey) {
      ...
      case "div": {
        ...
        return {
          key: factKey(f), factKey: factKey(f), weakKey: "weakDiv",
      ...
      default: {
        ...
        return {
          key: factKey(f), factKey: factKey(f), weakKey: "weak",
